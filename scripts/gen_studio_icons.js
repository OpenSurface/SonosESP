 ...
 *
 * ── Output format ───────────────────────────────────────────────────────────
 * Matches what lv_font_conv emits for this project (see lv_font_mdi_16.c):
 * 4bpp, uncompressed, glyph bitmaps packed as a CONTINUOUS bitstream with no
 * per-row padding, byte-aligned only at each glyph's end. adv_w is in 1/16 px.
 *
...
// 8-bit alpha -> 4bpp, packed as one continuous bitstream per glyph.
function pack4bpp(data) {
  const out = [];
  let acc = 0, half = false;
  for (let i = 0; i < data.length; i++) {
    const v = (data[i] + 8) >> 4;            // round to nearest of 16 levels
    const nib = v > 15 ? 15 : v;
    if (!half) { acc = nib << 4; half = true; }
    else { out.push(acc | nib); half = false; }
  }
  if (half) out.push(acc);                    // pad the final nibble
  return out;
}
