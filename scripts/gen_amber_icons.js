/**
 * Amber icon font generator.
 *
 * Rasterises the icon <symbol> definitions from the Claude Design canvas
 * ("SonosESP Amber.dc.html" and "SonosESP Boot + Screensaver.dc.html") into
 ...
 *
 * Usage:  node scripts/gen_amber_icons.js <canvas-dir> [outdir]
...
// the MDI range (U+F0000+) and of Latin-Ext, so a Amber font can carry a text
...
const BATTERY_SYMBOLS = {
  ...
  // Sleep timer (issue #173). Lucide's moon, a little smaller and lower so a
  // "z" fits in its bite: a bare moon reads as "night", the z says "timer".
  // The group's stroke-width is 1.7 / 0.86, so the moon's line matches.
  'st-sleep':    { attrs: BATTERY_STROKE,
                   body: '<g transform="translate(0.5 3) scale(0.86)" stroke-width="1.98">' +
                         '<path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></g>' +
                         '<path d="M15 2.5h5.5L15 8h5.5"/>' },
};
// Appended only - an existing glyph's codepoint must never move.
...
// AMB_IC_REFRESH " Scan", and on btn_play, whose glyph updateUI() writes as MDI.
//
...
function emitFont(name, px, icons, symbols, cpBase = CP_BASE) {
  ...
  s += '/*******************************************************************************\n';
  s += ` * Size: ${px} px\n * Bpp: 8\n`;
  s += ' * GENERATED by scripts/gen_amber_icons.js from the Claude Design canvas.\n';
...
if (process.argv[2] === '--battery') {
  ...
  let bh = '';
  bh += '/**\n * Battery (issue #165) and sleep-timer (issue #173) icon codepoints - GENERATED by\n';
  ...
  BATTERY_ORDER.forEach((id, i) => {
    // bt-full -> AMB_BAT_FULL, st-sleep -> AMB_ST_SLEEP
    const def = id.startsWith('bt-')
      ? 'AMB_BAT_' + id.slice(3).toUpperCase().replace(/-/g, '_')
      : 'AMB_' + id.toUpperCase().replace(/-/g, '_');
...
if (!canvasDir) {
  console.error('usage: node scripts/gen_amber_icons.js <canvas-dir> [outdir]');
  process.exit(1);
}
...
for (const px of WX_SIZES) {
  const name = `lv_font_amber_wx_${px}`;
...
function defName(id) {
  ...
  if (!m) return 'AMB_IC_' + id.toUpperCase();
  return (m[1] === 'sc' ? 'AMB_SC_' : 'AMB_IC_') + m[2].toUpperCase().replace(/-/g, '_');
}
...
h += ' * the matching lv_font_amber_* size as its font.\n */\n';
h += '#ifndef AMBER_ICONS_H\n#define AMBER_ICONS_H\n\n#include "lvgl.h"\n';
h += '#include "ui_icons.h"   // for the MDI-substituted entries below\n\n';
h += 'LV_FONT_DECLARE(lv_font_amber_16);\n';
h += 'LV_FONT_DECLARE(lv_font_amber_24);\n';
h += 'LV_FONT_DECLARE(lv_font_amber_32);\n';
h += 'LV_FONT_DECLARE(lv_font_amber_40);\n';
h += 'LV_FONT_DECLARE(lv_font_amber_wx_32);\n';
h += 'LV_FONT_DECLARE(lv_font_amber_wx_64);\n\n';
for (const m of manifest) {
...
wxIcons.forEach((id, i) => {
  h += `#define ${('AMB_WX_' + id.replace('sc-', '').toUpperCase()).padEnd(20)} "${utf8Escape(CP_BASE + i)}"   // in lv_font_amber_wx_*\n`;
});
h += '\n#endif // AMBER_ICONS_H\n';
fs.writeFileSync(path.join('include', 'amber_icons.h'), h);
console.log('include/amber_icons.h  ' + manifest.length + ' defines');
