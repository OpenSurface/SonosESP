...
/* Panel-specific init for the GUITION JC1060P470C, carried over from the
 * previous driver. Upstream ships only SLPOUT+DISPON, which lights the panel
 * but leaves it untuned - on this panel that showed as a washed-out screen
 * that fades (#113). The TIMINGS above are upstream's; only this table is ours. */
static const jd9165_lcd_init_cmd_t vendor_specific_init_default[] = {
/* Init sequence for the JC1060P470C_I_W_Y **New Panel**, taken verbatim from
 * GUITION's own demo package (JC1060P470C_I_W_Y.zip, Demo_Arduino/
 * JC1060P470C_I_W_Y_New_Panel/.../esp_lcd_jd9165.c). GUITION ship separate
 * New_Panel and Old_Panel demos for the same SKU - the panels need different
 * init sequences and timings. Our previous table was the Old_Panel one. */
